...
## 은닉층(Hidden Layer)과 노드(Node) 정리

### 기본 개념

| 용어       | 설명 |
|------------|------|
| **은닉층** | 입력층과 출력층 사이에 있는 레이어. 입력 데이터를 처리하고 다음 층으로 전달하는 중간 계산 단계 |
| **노드**   | 은닉층 안에 있는 하나의 뉴런. 입력을 받아 가중합을 계산하고, 활성화 함수로 출력함 |
...
| 항목       | 은닉층 | 노드 |
|------------|--------|------|
| **의미**     | 층의 개수 (깊이, depth) | 각 층의 너비 (width) |
| **역할**     | 계산 단계를 여러 단계로 나눔 | 각 층 내에서 다양한 특징 추출 |
| **영향력**   | 표현 깊이를 증가시킴 (고차 추상화) | 표현 다양성을 증가시킴 (세밀한 분류) |
| **복잡도 증가** | 층이 많을수록 깊은 구조 → 복잡한 문제 가능 | 노드가 많을수록 더 풍부한 패턴 인식 가능 |
...
| 항목 | 증가 효과 | 주의할 점 |
|------|-----------|------------|
| **은닉층 수 증가** | 더 복잡한 함수 학습 가능<br>고차원 개념 표현 | 과적합, 기울기 소실, 느린 학습 가능성 |
| **노드 수 증가**   | 더 많은 특징 학습 가능<br>결정 경계 복잡화 가능 | 계산량 증가, 과적합 가능성 |
...
| 항목 | 필요 조건 |
|------|-----------|
| **은닉층** | 1개면 충분 (비선형 함수 사용) |
| **노드 수** | 최소 2개, 3~4개면 더 안정적 학습 가능 |
