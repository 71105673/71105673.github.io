...
| 모델 번호 | 모델 구조 요약 | Validation Accuracy (최종) | Validation Loss (최종) | 특징 요약 |
|-----------|------------------|-----------------------------|--------------------------|-------------|
| **1** | 128 → 64 → 10 (2-layer, ReLU) | **96.69%** | 0.1117 | 기본 구조, 안정적인 성능 |
| **2** | 128 → 64 → 32(ReLU) → 10 | **96.70%** | 0.1143 | 은닉층 추가로 약간의 성능 향상 |
| **3** | 128 → 64 → 32(ReLU) → Dropout(0.5) → 10 | **96.69%** | 0.1196 | 과적합 방지 목적. Accuracy는 비슷하나 Loss는 상승 |
| **4** | 128 → 64 → 32(LeakyReLU) → Dropout(0.5) → 10 | **96.83%** | 0.1086 | 가장 높은 성능, 가장 낮은 Loss 달성 |

---

### 📊 분석

- **기본 모델 (1)** 은 단순한 2-layer 구조임에도 불구하고 높은 정확도(96.69%)를 달성함.
- **모델 2 (ReLU 추가)** 는 은닉층이 추가되어 표현력이 증가했지만, 성능 변화는 크지 않음.
- **모델 3 (Dropout 추가)** 는 과적합을 방지했으나, 오히려 validation loss 증가로 일반화 성능 저하가 의심됨.
- **모델 4 (LeakyReLU 적용)** 는 ReLU의 단점(죽은 뉴런 현상)을 보완하여 가장 뛰어난 accuracy와 loss를 기록.

### 💡 고찰

- 단순한 layer 추가는 성능 개선에 한계가 있으며, **활성화 함수와 정규화 전략의 조합**이 성능에 큰 영향을 미침.
- **Dropout**은 과적합이 우려되는 복잡한 모델에서 효과적이지만, 작은 모델에서는 오히려 성능을 떨어뜨릴 수 있음.
- **LeakyReLU**는 ReLU의 한계를 극복할 수 있는 좋은 대안으로, **정보 손실 없이 gradient 흐름을 유지**해 학습 안정성과 성능을 동시에 확보함.
- 따라서 모델 최적화를 위해서는 **단순한 layer 수의 증가보다**, 활성화 함수, 정규화, 하이퍼파라미터 튜닝의 조화가 중요함.
