...
|-----------|------------------|-----------------------------|--------------------------|-------------|
| **1** | 128 → 64 → 10 (2-layer, ReLU) | **96.69%** | 11.17% | 기본 구조, 안정적인 성능 |
| **2** | 128 → 64 → 32(ReLU) → 10 | **96.70%** | 11.43% | 은닉층 추가로 약간의 성능 향상 |
| **3** | 128 → 64 → 32(ReLU) → Dropout(0.5) → 10 | **96.69%** | 11.96% | 과적합 방지 목적. Accuracy는 비슷하나 Loss는 상승 |
| **4** | 128 → 64 → 32(LeakyReLU) → Dropout(0.5) → 10 | **96.83%** | 10.86% | 가장 높은 성능, 가장 낮은 Loss 달성 |
